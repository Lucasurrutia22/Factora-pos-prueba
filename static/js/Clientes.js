...
(function() {
        ...
        try {
            ...
            if (result.success) {
                customers = result.data.map(c => ({
                    ...
                    city: '',
                    pais: c.pais || '',
                    registrationDate: c.fecha_creacion ? new Date(c.fecha_creacion).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
                }));
    ...
    window.handleCustomerSubmit = async function(e) {
        ...
        const pais = 'Chile'; // Default

        console.log('Datos del formulario:', { name, pais });
